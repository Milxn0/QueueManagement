...
function getRange(range: "today" | "month") {
    ...
    if (range === "today") {
        ...
        return { startISO: start.toISOString(), endISO: end.toISOString(), label: "month" };
    }
}
...
export async function GET(req: NextRequest) {
    try {
        ...
        let query = supabase
            .from("reservations")
            .select("id,user_id,reservation_datetime,partysize,queue_code,status,created_at,table_id")
        ...
        const filename = `reservations_${label}_${new Date().toISOString().slice(0, 10)}.xlsx`;
